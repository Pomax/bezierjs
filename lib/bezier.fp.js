...
Bezier.dimlen = Bezier.dimLen = function(curve) {
  return 'z' in curve[0] ? 3 : 2;
};

// TODO: propose errorThreshold upstream
Bezier.isLinear = function(curve, errorThreshold) {
  errorThreshold = errorThreshold || 0.0001;
  var order = Bezier.order(curve);
  var a = utils.align(curve, {p1:curve[0], p2:curve[order]});
  for(var i=0; i<a.length; i++) {
    if(abs(a[i].y) > errorThreshold) {
      return false;
    }
  }
  return true;
...
// This is the same logic executed on each bezier#update.
// It returns an array of 'dpoints'.
...
Bezier.get = Bezier.compute = function(curve, t) {
  ...
  var dCpts = curve.map(function(point) {
    ...
    while(t2 <= 1) {
      for(t2=t1+step; t2<=1+step; t2+=step) {
        ...
        if(!Bezier.isSimple(segment)) {
          ...
          segment = Bezier.split(p1,t1,t2);
          segment[0]._t1 = utils.map(t1,0,1,p1[0]._t1 || 0, p1[0]._t2 || 1);
          segment[0]._t2 = utils.map(t2,0,1,p1[0]._t1 || 0, p1[0]._t2 || 1);
          pass2.push(segment);
          t1 = t2;
          break;
        }
...
Bezier.outlineshapes = Bezier.outlineShapes = function(curve, d1, d2, curveIntersectionThreshold) {
  d2 = d2 || d1;
...
Bezier.selfintersects = Bezier.selfIntersects = function(curve, curveIntersectionThreshold) {
  var reduced = Bezier.reduce(curve);
  ...
  for(i=0; i<len; i++) {
    ...
    result = Bezier.curveIntersects(left, right, curveIntersectionThreshold);
    Array.prototype.push.apply(results, result);
  }
  return results;
};

Bezier.curveintersects = Bezier.curveIntersects = function(c1, c2, curveIntersectionThreshold) {
  var pairs = [];
