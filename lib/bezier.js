...
(function() {
  ...
  var utils = {
    ...
    makeshape: function(forward, back, order) {
      var start  = this.makeline(back.points[order], forward.points[0]);
      var end    = this.makeline(forward.points[order], back.points[0]);
      var shape  = {
        startcap: start,
        forward: forward,
        back: back,
  ...
  Bezier.prototype = {
    ...
    scale: function(d) {
      ...
      var points=this.points,np=[],p,order=this.order;
    ...
    outline: function(d1, d2) {
      ...
      var fs = fcurves[0].points[0],
          fe = fcurves[len-1].points[order],
          bs = bcurves[len-1].points[order],
    ...
    outlineshapes: function(d1,d2) {
      ...
      for(var i=1, len=outline.length; i < len/2; i++) {
        var shape = utils.makeshape(outline[i], outline[len-i], this.order);
